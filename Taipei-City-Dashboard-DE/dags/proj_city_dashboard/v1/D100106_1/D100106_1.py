...
def _extract_db_info(default_table, engine):
    ...
    # Filter columns
    keep_col = ["tname", "wkb_geometry"]
    existing_data = existing_data[keep_col]
    return existing_data


def D100106_1(**kwargs):
    """
    ...
    # Set condition
    condition = (data["district"].str.contains("區")) & (data["gender"] == "計")
    data = data[condition]
    ...
    existing_data = _extract_db_info(district_table, engine)
    gdata = data.merge(existing_data, left_on="district", right_on="tname", how="left")
    gdata.drop(columns=["tname"], inplace=True)
...
dag = CommonDag(proj_folder="proj_city_dashboard/v1", dag_folder="D100106_1")
dag.create_dag(etl_func=D100106_1)
